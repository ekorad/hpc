...
void InvMixColumns(state State){
	...
	for(i=0;i<Nb;i++)
		for(j=0;j<Nb;j++) temp[i][j]=State[i][j];
	for(j=0;j<Nb;j++){
		unsigned char tmp=xtime(xtime(xtime(temp[0][j]^temp[1][j]^temp[2][j]^temp[3][j])));
		State[0][j]=xtime(xtime(temp[0][j]^temp[2][j]))^temp[1][j]^temp[2][j]^temp[3][j]^xtime(temp[0][j]^temp[1][j])^tmp;
		State[1][j]=xtime(xtime(temp[1][j]^temp[3][j]))^temp[0][j]^temp[2][j]^temp[3][j]^xtime(temp[1][j]^temp[2][j])^tmp;
		State[2][j]=xtime(xtime(temp[2][j]^temp[0][j]))^temp[0][j]^temp[1][j]^temp[3][j]^xtime(temp[2][j]^temp[3][j])^tmp;		
		State[3][j]=xtime(xtime(temp[3][j]^temp[1][j]))^temp[0][j]^temp[1][j]^temp[2][j]^xtime(temp[3][j]^temp[0][j])^tmp;
	}
}
